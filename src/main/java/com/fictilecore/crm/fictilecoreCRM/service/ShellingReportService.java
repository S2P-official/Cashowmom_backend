...
public class ShellingReportService {
        ...
        public List<ShellingDaySummaryDTO> getSummaryBetweenDates(
                ...
                List<ShellingReport> reports = shellingReportRepository.findByTenant_IdAndDateBetween(tenantId, from,
                                to);

                Map<LocalDate, List<ShellingReport>> grouped = reports.stream()
                                .collect(Collectors.groupingBy(ShellingReport::getDate));
        ...
        public MonthlyShellingResponse getMonthlyReport(
                ...
                Map<String, List<ShellingReportDTO>> grouped = shellingReportRepository
        ...
        public List<ShellingReportResponseDTO> getReportsByTenantEmployeeAndDate(
                Long tenantId,
                Long employeeId,
                LocalDate date) {
    // Use the fixed repository query
    List<ShellingReport> reports = shellingReportRepository
            .findReportsByTenantEmployeeAndDate(tenantId, employeeId, date);

    // Convert to DTO
    return reports.stream()
            .map(ShellingReportResponseDTO::fromEntity)
            .toList();
}
}
